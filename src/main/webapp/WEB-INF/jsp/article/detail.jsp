...
<div class="detail-background con">
	...
	<!-- 게시물 수정 삭제 리스트 -->
		...
		<c:if test="${isLogined}">
			...
			<form class="article-apply-box table-box con form1"
							...
							<td>
								<div class="form-control-box">
									<textarea maxlength="300" name="body" placeholder="내용을 입력해주세요."
										class="height-300" style="resize: none; width: 420px;"></textarea>
								</div>
							</td>
						</tr>
						<c:forEach var="i" begin="1" end="3" step="1">
		...
		<script>
			...
			function ReplyList__submitModifyForm(form) {
				...
				var startUploadFiles = function() {
					var needToUpload = false;
					if (needToUpload == false) {
						needToUpload = fileInput1
								&& fileInput1.value.length > 0;
					}
					if (needToUpload == false) {
						needToUpload = deleteFileInput1
								&& deleteFileInput1.checked;
					}
					if (needToUpload == false) {
						needToUpload = fileInput2
								&& fileInput2.value.length > 0;
					}
					if (needToUpload == false) {
						needToUpload = deleteFileInput2
								&& deleteFileInput2.checked;
					}
					if (needToUpload == false) {
						needToUpload = fileInput3
								&& fileInput3.value.length > 0;
					}
					if (needToUpload == false) {
						needToUpload = deleteFileInput3
								&& deleteFileInput3.checked;
					}
					if (needToUpload == false) {
					...
					if (data.resultCode && data.resultCode.substr(0, 2) == 'S-') {
						...
						$(
								'.reply-list-box tbody > tr[data-id="' + id
										+ '"] .img-box').empty();
						if (data && data.body
								&& data.body.file__common__attachment) {
			...
			function ReplyList__showModifyFormModal(el) {
				...
				for (var fileNo = 1; fileNo <= 3; fileNo++) {
					...
					var imgName = 'reply__' + id + '__common__attachment__'
							+ fileNo;
					var $imgBox = $('.reply-list-box [data-img-name="'
							+ imgName + '"]');
					if ($imgBox.length > 0) {
						$('.reply-modify-form-modal .img-box-file-' + fileNo)
								.append($imgBox.html());
			...
			function ReplyList__drawReply(reply) {
				...
				html += '<td>' + reply.id + '</td>';
				html += '<td class="visible-on-md-up">' + reply.regDate
						+ '</td>';
				html += '<td class="visible-on-md-up">' + reply.extra.writer
						+ '</td>';
				html += '<td>';
				html += '<div class="reply-body">' + reply.forPrintBody
						+ '</div>';
