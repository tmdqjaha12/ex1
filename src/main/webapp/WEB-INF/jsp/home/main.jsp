...
<script>
	...
	function selectMove() {
		playerMove = prompt(
				"You have 4 moves, " + moves[0].move + ", " + moves[1].move
						+ ", " + moves[2].move + " and " + moves[3].move
						+ ", which move do you want to use?").toUpperCase();
		if (playerMove === "") {
			selectMove();
		} else {
			switch (playerMove) {
	...
	function wildPokemonAttack() {
		if (pokemonHealth > 0) {
			...
			alert("Wild " + pokemonStats[wildPokemonid].type + " uses "
					+ moves[moveid].move + " dealing " + damage + " damage!");
			attackLoop();
		} else {
	...
	function playerAttack() {
		alert("Player uses " + moves[playerMove].move + " dealing " + damage
				+ " damage!");
		if (wildPokemonHealth > 0) {
			wildPokemonHealth = wildPokemonHealth - damage;
			alert(pokemonStats[wildPokemonid].type + " has "
					+ wildPokemonHealth + " health remaining!");
			playerTurn = false;
			wildPokemonFaint()
		} else {
